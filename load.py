import numpy as np
import torch
# from math import radians, cos, sin, asin, sqrt # CHANGED
import haversine as hs # CHANGED
import joblib
...
def process_traj(dname):
    ...
    num_user = len(set(data[:, 0]))  # max id of users, i.e. NUM
    data_user = data[:, 0]  # user_id sequence in data
    trajs, labels, mat1, mat2t, lens = [], [], [], [], []
    u_max, l_max = np.max(data[:, 0]), np.max(data[:, 1])

    for u_id in range(1,num_user+1): # CHANGED
        # if u_id == 0:  # skip u_id == 0 # CHANGED
        #     continue # CHANGED
        init_mat1 = np.zeros((max_len, max_len, 2))  # first mat (M, M, 2)
        init_mat2t = np.zeros((max_len, max_len))  # second mat of time (M, M)
        user_traj = data[np.where(data_user == u_id)].copy()  # find all check-ins of u_id # CHANGED
        # user_traj = user_traj[np.argsort(user_traj[:, 2])].copy()  # sort traj by time # CHANGED
        ...
        if len(user_traj) > max_len + 1:  # consider only the M+1 recent check-ins
            ...
            user_traj = user_traj[-max_len-1:]  # (*M+1, [u, l, t]) # take last 101 records
...
def rst_mat1(traj, poi):
    ...
    for i, item in enumerate(traj):
        for j, term in enumerate(traj):
            ...
            # mat[i, j, 0] = haversine(lon1=poi_item[2], lat1=poi_item[1], lon2=poi_term[2], lat2=poi_term[1]) # CHANGED
            mat[i, j, 0] = hs.haversine((poi_item[1],poi_item[2]),(poi_term[1],poi_term[2])) # CHANGED
...
def rs_mat2s(poi, l_max):
    ...
    for i, loc1 in enumerate(candidate_loc):
        ...
        for j, loc2 in enumerate(candidate_loc):
            ...
            # mat[i, j] = haversine(lon1=poi1[2], lat1=poi1[1], lon2=poi2[2], lat2=poi2[1]) # CHANGED
            mat[i, j] = hs.haversine((poi1[1],poi1[2]), (poi2[1],poi2[2])) # CHANGED
    return mat  # (L, L)
